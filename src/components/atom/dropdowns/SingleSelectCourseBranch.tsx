...
const SingleSelectCourseBranchDropdown = ({
  ...
  useEffect(() => {
    classValue ? setSelectedCourse(classValue) : setSelectedCourse("")
    branchValue ? setSelectedBranch(branchValue) : setSelectedBranch("")
  }, [classValue, branchValue])

  useEffect(() => {
    value ? setSelectedCourse(optionsData[0]) : setSelectedCourse("")
    classValue ? setSelectedCourse(classValue) : setSelectedCourse("")
    branchValue ? setSelectedBranch(branchValue) : setSelectedBranch("")
  ...
  const handleCourseSelect = (course: string) => {
    ...
    setSelectedBranch(""); // reset branch on course change
    setCourseDropdownOpen(false);
  ...
  const availableBranches = selectedCourse ? (branchMap?.[selectedCourse] ?? courseBranchMap[selectedCourse] ?? []) : [];
      ...
      <div className="relative" ref={courseRef}>
        <button
          ...
          onClick={() => {
            ...
            if (!courseDropdownOpen) {
              onDropdownOpen?.(); // Call the prop only when opening
            }
          }}
        ...
        >
          <span className="truncate text-sm">
            {selectedCourse || `Select ${placeholder}`}
          </span>
