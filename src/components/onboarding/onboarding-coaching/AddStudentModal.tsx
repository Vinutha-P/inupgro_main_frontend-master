...
const AddStudentModal = ({ isOpen,
	...
	onAddStudent: (faculty: typeof formData) => void;
}) => {
	...
	const handleSubmit = () => {
		...
		if (isValid) {
			onAddStudent(formData); // Add to parent list
			setFormData({
				fullName: "",
				age: "",
				examType: "",
	...
	return (
		<div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
			<div className="bg-white rounded-xl w-full max-w-2xl p-4 relative shadow-lg">
				...
				<h3 className="text-[0.6rem] font-bold text-deepBlue mt-5">Student Info</h3>
				<div className="mt-6 flex flex-col md:flex-row gap-3">
					...
					<div className="w-full md:w-[60%] grid grid-cols-1 md:grid-cols-2 gap-3">
						{["fullName", "age", "examType", "marks"].map((field, idx) => (
							<div key={idx}>
								<label htmlFor={field} className="text-[0.6rem] font-semibold text-deepBlue">
									{field === "examType" ? "Exam Type" : field.charAt(0).toUpperCase() + field.slice(1)}
								</label>
								<input
									id={field}
									type="text"
								...
								/>
								{field === "marks" && (
									<span className="absolute right-20 top-[69%] -translate-y-1/2 text-[0.8rem] text-gray-700 pointer-events-none">%</span>
								)}
								{(field === "fullName" || field === "age") && errors[field] && (
				...
				<div className="flex justify-end mt-6">
					<button onClick={handleSubmit} className="bg-primaryLight text-white text-[0.6rem] px-5 py-2 rounded hover:bg-blue-600 transition">
						Save & Continue
					</button>
				</div>
			</div>
